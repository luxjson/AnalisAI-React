<img src="https://github.com/luxjson/AnalisAI-React/blob/main/frontend/public/IMG/gitlogo.png" width="100px" align="left">

### `AnalisAI - With React + Vite`

![Status](https://img.shields.io/badge/STATUS-OPERATIONAL-green)
![Environment](https://img.shields.io/badge/ENV-SENAI_CIC-red)
![Stack](https://img.shields.io/badge/STACK-PERN-blue)

**AnalisAI** é uma plataforma de gestão pedagógica projetada para converter dados brutos de avaliações em inteligência visual, monitorando competências e desempenho acadêmico em tempo real.

### Estrutura do Subsistema (`/src`)

- [`src/`](./src) - Pasta de arquivos do site.
- [`public/`](./public) - Ativos estáticos e estilização Neo-brutalista.
- [`server/`](./server) - Servidor em que o site está sendo rodado.

<div align="center"> 
<sub>Built with ☕ and 🌙 by LukzST</sub>
</div>
